...
SUPPORTED_PRETRAIN_EXTENSIONS = {'.npy', '.npz', '.csv', '.txt', '.parquet', '.arrow', '.tsf'}
    ...
    def __read_data__(self):
        for root, dirs, files in os.walk(self.root_path):
            for file in files:
                ...
                if file_ext not in SUPPORTED_PRETRAIN_EXTENSIONS:
                    continue
                try:
                    if file_ext == '.npy':
                        ...
                        if load_dataset is None:
                            raise ImportError("Install `datasets` to read parquet pretraining files.")
                        ...
                        if load_dataset is None:
                            raise ImportError("Install `datasets` to read arrow pretraining files.")
                    ...
                    warnings.warn(f"Skipping unreadable pretraining file {subdataset_path}: {exc}")
                    continue
               
                self.scaler = StandardScaler()
                dataset_list = data if file_ext in {'.parquet', '.arrow', '.tsf'} else [data]
                ...
                try:
                    ...
                    warnings.warn(f"Skipping unusable pretraining file {subdataset_path}: {exc}")
                    continue
        ...
        if not self.n_window_list:
            raise FileNotFoundError(
                f"No usable pretraining data found under {self.root_path}. "
                "Place the open-domain datasets under ./dataset/pretrain/ as described "
                "in dataset/README.md, or run `python -m data_provider.data_manifest --root ./dataset`."
            )
